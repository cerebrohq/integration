...
DEFAULT_URL = 'http://s2:81/dapi/rpc.php'
...
class Database():
	...
	* :py:meth:`users()                          <py_cerebro.database.Database.users>`
	...
	def connect(self, db_user, db_password, primary_url=DEFAULT_URL, secondaryy_url=''):
		"""
		...
		payload = {
			'method': 'sessionStart',
			'jsonrpc': '2.0',
			'params': [self.db_user, self.db_password, 1],
			'id': self.query_id
		}
		...
		if res is not None:
			...
			self.sid = int(res["result"][len(res["result"]) - 1]["token"])
			#self.db_url_primary = res["result"][len(res["result"]) - 1]["server"]["instances"][0]["addr_jrpc"] + u"/rpc.php"
			self.db_url_primary = 'http://s2:81/dapi/rpc.php'
			self.db_url_secondary = 'http://s2:81/dapi/rpc.php'
		else:
